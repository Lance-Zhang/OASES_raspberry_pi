...
PID_forback = PID(10,0,0) # adjust the forward / backward position
PID_side = PID(10,0,0) # adjust the sideward position
print("Init PID.")
...
while True:
    ...
    # set target
    now = time.time() - initial_time
    x_t = radius * math.cos(A * now)
    y_t = radius * math.sin(A * now)
    yaw_t = 0 #
